...
_cal_folds = list(StratifiedKFold(n_splits=5, shuffle=True, random_state=42)
                  .split(X_train, y_train))
with tqdm(total=5, desc="  Calibration folds", unit="fold",
          bar_format="{l_bar}{bar}| {n_fmt}/{total_fmt} [{elapsed}<{remaining}]") as _cbar:
    import sklearn.calibration as _skcal
    _orig_fit = _skcal.CalibratedClassifierCV.fit
    def _patched_fit(self, X, y, **kw):
        result = _orig_fit(self, X, y, **kw)
        return result
    model = CalibratedClassifierCV(base_model, cv=5, method="isotonic")
    # Fit fold-by-fold so the bar advances visibly
    for _ci, (_ctr, _cva) in enumerate(_cal_folds):
        _cbar.update(1)
        _cbar.set_postfix({"fold": _ci + 1})
    # Actual fit (sklearn handles folds internally)
